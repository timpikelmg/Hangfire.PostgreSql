...
{
  public class PostgreSqlTimeoutJobFacts
  {
    ...
    public PostgreSqlTimeoutJobFacts()
    {
    ...
    {
      ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => new PostgreSqlTimeoutJob(null, 1, JobId, Queue, _fetchedAt));
    ...
    {
      ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => new PostgreSqlTimeoutJob(_storage, 1, null, Queue, _fetchedAt));
    ...
    {
      ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => new PostgreSqlTimeoutJob(_storage, 1, JobId, null, _fetchedAt));
    ...
    {
      ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => new PostgreSqlTimeoutJob(_storage, 1, JobId, Queue, null));
    ...
    {
      PostgreSqlTimeoutJob timeoutJob = new(_storage, 1, JobId, Queue, _fetchedAt);

      Assert.Equal(1, timeoutJob.Id);
      Assert.Equal(JobId, timeoutJob.JobId);
      Assert.Equal(Queue, timeoutJob.Queue);
      Assert.Equal(_fetchedAt, timeoutJob.FetchedAt);
    }
    ...
    public void RemoveFromQueue_ReallyDeletesTheJobFromTheQueue()
    {
      ...
      PostgreSqlTimeoutJob processingJob = new(_storage, id, "1", "default", _fetchedAt);
    ...
    public void RemoveFromQueue_DoesNotDelete_UnrelatedJobs()
    {
      ...
      PostgreSqlTimeoutJob timeoutJob = new PostgreSqlTimeoutJob(_storage, 999, "1", "default", _fetchedAt);

      // Act
      timeoutJob.RemoveFromQueue();
    ...
    public void Timer_UpdatesFetchedAtColumn()
    {
      _storage.UseConnection(null, connection => {
        ...
        using (var processingJob = new PostgreSqlTimeoutJob(_storage, id, "1", "default", fetchedAt))
        {
    ...
    public void RemoveFromQueue_AfterTimer_RemovesJobFromTheQueue()
    {
      _storage.UseConnection(null, connection => {
        // Arrange
        long id = CreateJobQueueRecord(_storage, "1", "default", _fetchedAt);
        using (PostgreSqlTimeoutJob processingJob = new PostgreSqlTimeoutJob(_storage, id, "1", "default", _fetchedAt))
        {
          Thread.Sleep(TimeSpan.FromSeconds(10));
          processingJob.DisposeTimer();
    ...
    public void RequeueQueue_AfterTimer_SetsFetchedAtValueToNull()
    {
      _storage.UseConnection(null, connection => {
        // Arrange
        long id = CreateJobQueueRecord(_storage, "1", "default", _fetchedAt);
        using (var processingJob = new PostgreSqlTimeoutJob(_storage, id, "1", "default", _fetchedAt))
        {
          Thread.Sleep(TimeSpan.FromSeconds(10));
          processingJob.DisposeTimer();
